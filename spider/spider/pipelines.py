    ...
    def process_item(self, item, spider):
        ...
        if item['goods']:
            if item['goods']['price'] != item['price'] or item['goods']['original_price'] != item['originalPrice'] or item['goods']['name'] != item['name']:
                self.db.execute("UPDATE le_goods SET isshow=1,name=%s,price=%s,original_price=%s, uptime=%s,expiry_time=%s, site_id=%s ,`img`=%s, `deal_img`=%s,`desc_bigpic`=%s, `oldimg`=%s, `small_pic`=%s,`desc_oldimg`=%s,`bigpic`=%sWHERE goods_id=%s",[item['name'],item['price'],item['originalPrice'],int(time.time()),item['ExpiryTime'], item['site_id'],img,img,'',old_pic,small_pic,'',big_pic,item['goods']['goods_id']])
            else:
                self.db.execute("UPDATE le_goods SET isshow=1,uptime=%s,expiry_time=%s,site_id=%s ,`img`=%s, `deal_img`=%s,`desc_bigpic`=%s, `oldimg`=%s, `small_pic`=%s,`desc_oldimg`=%s,`bigpic`=%s WHERE goods_id=%s",[int(time.time()),item['ExpiryTime'], item['site_id'], img,img,'',old_pic,small_pic,'',big_pic,item['goods']['goods_id']])
        else:

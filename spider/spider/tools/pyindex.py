...
for goods in goods_list:
    conn.index({'title':goods['title'], 'brand':goods['brand'], 'category':goods['category'],'price':goods['price'],'add_time':goods['add_time'],'unique_id':goods['unique_id']}, 'godos-index', 'goods', i, True)
    i = i + 1
                 #向human的man中添加索引
conn.refresh()
